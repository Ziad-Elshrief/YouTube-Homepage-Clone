...
export default function SideBar() {
  ...
  return (
    <>
      ...
      >
        <div className="lg:hidden pt-2 px-2 pb-4 sticky top-0 bg-white">
...
function SmallSidebarItem({ Icon, title, url }: SmallSidebarItemProps) {
  return (
    ...
    >
      <Icon className="size-6" /> <div className="text-sm">{title}</div>
    </a>
  );
}
    ...
    <div className="">
      ...
      {showExpandedButton && (
        <Button
          ...
          variant="ghost"
          className="w-full flex items-center rounded-lg p-3 gap-4"
        >
...
function LargeSidebarItem({
  ...
  return (
    <a
      ...
      className={twMerge(
        buttonStyles({ variant: "ghost" }),
        `w-full flex items-center rounded-lg p-3 gap-4 ${
          isActive ? "font-bold bg-neutral-100 hover:bg-secondary" : undefined
        }`
      )}
    >
      {typeof IconOrUrl === "string" ? (
        <img src={IconOrUrl} className="size-6 rounded-full" />
      ) : (
        <IconOrUrl className="size-6" />
      )}
